...
class Edit extends Component {
  constructor(){
    ...
    this.state ={
      ...
      password: '',
      vendor: '',
      vendorPath: sessionStorage.getItem("path")
    }
  ...
  this.updateInfoName = ()=>{
    ...
    let found = sessionStorage.getItem("path");
    if(found === null){
      found = "01"
    }
    const vendRef = firebase.database().ref().child('Vendor').child(found);
  ...
  this.updateInfoImage = ()=>{
   //assign root ref for DB
   let locRef = sessionStorage.getItem("lastname");
   let found = sessionStorage.getItem("path");
   if(found === null){
     found = "01"
   }
  ...
  componentDidMount(){
    ...
    console.log('Vendor Path ' + found);
    console.log('Vendor Ref ' + vendRef);
